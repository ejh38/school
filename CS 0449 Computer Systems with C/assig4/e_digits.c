...
int main(int argc, char **argv) {
	...
	FILE *f = fopen("/dev/e_driver", "rt");	// open up the file
	if (f == NULL) {
	...
	fread(return_string, 1, amount, f);	// and then read them all in

	printf("Here come %d digits of e, from index %d to %d:\n%s",
														 amount, start_val, end_val, return_string);
	fclose(f);
