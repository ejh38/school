 ...
 * zps6@pitt.edu
 * Project 1 - Part 2
 * Opens a song file, looks for an id3 tag, and either edits it in place,
 * prints it out, or appends a new one to the end.
 * Has some slight error checking, for an invalid filename, poorly formatted
 * command line args, but it DOES require shell quoting.
 * Also, if there would be overflow (35 characters given for a 30 character spot)
 * then only the first 30 characters are read in and put in the spot.
 ************************************************************************/
...
int main (int argc, char *argv[]) {	
	...
	short error;						// a flag for breaking out of the program
	...
	if (f == NULL) {
		...
		if (strncmp(value_string, "TAG", 3) == 0) {
			...
			if (strncmp(value_string, "TAG", 3) == 0) {
				...
				printf("\nNo id3tag found. Exiting the program.\n");
				no_tag_found = 1;
			}
		}
		...
		if (argc == 2 && no_tag_found != 1) {
			...
			if (no_tag_found) {
				// this fills the tag with empty bytes to make sure
				// there's no garbage for any reason
			...
			for (i = 2; i < argc; i += 2) {		// increment by 2 for field and value
				...
				if (strcmp(field_name, "-title") == 0) {		// check if it's "-title"
					...
					// if the field name was none of the above, then an error has occurred
					if (strncmp(argv[i], "-", 1) != 0) {
						// this error means they had no - before the field name
						printf("\nImproper formatting! Please use the format:\n");
						printf("\"id3tagEd FILENAME -FIELD VALUE -FIELD VALUE ...\"\n");
						printf("(Looks like you missed the '-' before '%s')\n", argv[i]);
						printf("\nExiting the program, please try again with proper formatting.\n");
						error = 2;
					}
					else {
						// and this error means they had a - but an improper field name
						printf("\nImproper formatting! Please use the format:\n");
						printf("\"id3tagEd filename -field value -field value ...\"\n");
			...
			if (!error) {								// this lets us skip to the finish if we hit an error
				if (no_tag_found) {				// go to the end if we're writing a new tag
					fseek(f, 0, SEEK_END);
				}
				else {
