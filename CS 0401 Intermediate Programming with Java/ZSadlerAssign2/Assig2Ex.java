...
public class Assig2Ex
{
	...
	public static void main(String [] args) throws IOException
	{
		...
		while (!exit)	// main loop for the program
		{		
		...
		Player P1 = new Player(name);	// construct the Player object
		...
		else
		{
			...
			Quiz2 game = new Quiz2("questions2.txt");	// creates a quiz object based on the questions file
			
			while (game.hasAQuestion())			// this loops through all the questions in questions.txt
			{
				System.out.println("\n" + game.getQuestion());
				answer = inScan.next();
				...
				if (answer.equalsIgnoreCase(correct))
					{
						...
						game.getHint();			// still called, it just returns null
					}

				// after they get the question right or wrong, this increments numRight or numWrong
				P1.rightOrWrong(answer.equalsIgnoreCase(correct));	
			}
			
			// construct a new PrintWriter in append mode
			PrintWriter savePlayer = new PrintWriter(new FileOutputStream("players.txt",true));
			savePlayer.println();			// skips to the next line
			
			//outputs the player's info to players.txt
	...
	// this outputs the results of the player and overall totals
