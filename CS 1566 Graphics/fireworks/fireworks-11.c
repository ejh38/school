...
void my_keyboard( unsigned char key, int x, int y ) 
{
  ...
  case '2': {
    make_explosion(&explosions[1], 200, 1, 64, .01, 1, test_sound, (&nozzles[1])->x, (&nozzles[1])->y + 1);
    
    break;
  }
  case '3': {
    make_explosion(&explosions[2], 350, 1, 64, .05, 1, test_sound, (&nozzles[2])->x, (&nozzles[2])->y + 1);
    
    break;
  }
  case '4': {
    make_explosion(&explosions[3], 1000, 1, 100, .05, 1, test_sound, (&nozzles[3])->x, (&nozzles[3])->y + 1);
    
    break;
  }
  case '5': {
    make_explosion(&explosions[4], 1500, 1, 64, .1, 1, test_sound, (&nozzles[4])->x, (&nozzles[4])->y + 1);
    
    break;
  }
  case '6': {
    make_explosion(&explosions[5], 600, 1, 64, .09, 1, test_sound, (&nozzles[5])->x, (&nozzles[5])->y + 1);
    
    break;
  }
  case '7': {
    make_explosion(&explosions[6], 2500, 1, 64, .3, 1, test_sound, (&nozzles[6])->x, (&nozzles[6])->y + 1);
...
void make_explosion(EXPLOSION *po, GLfloat radius, GLfloat density, 
  ...
  for (i = 0; i <= k - 1; i++) {
    for (j = 0; j <= k - 1; j++) {
      ...
      int t = (360.0/(float)(k - 1)  * j);    // theta
      int p = (180.0/(float)(k - 1) * i) - 90; // phi
