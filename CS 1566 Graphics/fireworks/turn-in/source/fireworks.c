...
{ 
  ...
  case 'E': {
  		...
  		BASS_ChannelPlay(sounds[16], TRUE);
  ...
  case 'R': {
  		...
  		//sparkler out of 4
  		BASS_ChannelPlay(sounds[17], TRUE);
  ...
  case 'T': {
  		...
  		//sparkler out of 5
  		BASS_ChannelPlay(sounds[18], TRUE);
  ...
  case 'Y': {
  		...
  		//sparkler out of 6
  		BASS_ChannelPlay(sounds[19], TRUE);
  ...
  case 'U': {
  		...
  		//sparkler out of 7
  		BASS_ChannelPlay(sounds[20], TRUE);
  ...
  case 'z': {
    ...
    if (music_playing) {
      BASS_ChannelPlay(sounds[9], FALSE);
    }
    else {
      BASS_ChannelPause(sounds[9]);
    }
  } break;
  ...
  case '7': {
    ...
    BASS_ChannelPlay(sounds[8], TRUE);
...
void make_explosion(EXPLOSION *po, GLfloat radius, GLfloat density, 
  ...
  if(shape == BOOM || shape == REGULAR || shape == SPARKLER
	  ...
	  for (i = 0; i <= k; i++) {
        ...
        (&(po -> particles[i*4 + j])) -> is_sparkler = 0; 
      }
      if (shape == WILLOW) {
      ...
      if (shape == SPARKLER) {
        ...
        (&(po -> particles[count - 1 + i*4 + j])) -> is_sparkler = 0; 
      }
      if (shape == WILLOW) {
  ...
  for (j = 0; j < MAX_FIREWORKS; j++) {
    ...
    if (curr_fuse -> hangtime > 0) {
      ...
      for (i = 0; i < curr_fuse->trail_size; i++) {
        ...
        if(curr_fuse->start_y==-1){
        	nozzle_change = (float)((rand() % (41)-20))/5000.;
          	cur->init_color[0]=cur->init_color[0]-1250*fall_time;
          	cur->init_color[1]=cur->init_color[1]-1250*fall_time;
          	cur->init_color[2]=cur->init_color[2]-1250*fall_time;
        }
        else{
        	nozzle_change=0;
        }
      ...
      if (curr_fuse -> counter < curr_fuse -> hangtime) {
      	...
      	if(cur_shape ==2){
      		...
      		if(cur_shape ==2){
      		special_rand = rand() % (NUM_SHAPES+1); 
      		if(special_rand ==0){
      			cur_shape = 2;
      		}
      		else if(special_rand==1){
      			cur_shape = TRIFORCE;
      		}
      		else if(special_rand==2){
      			cur_shape = ONE_UP;
      		}
      		else if(special_rand==3){
      			cur_shape = MUSHROOM;
      		}
      		else if(special_rand==4){
      			cur_shape = RECURSIVE;
      		}
      	}
      	}
      	...
      	if(cur_shape==1 || cur_shape == SPARKLER){
        ...
        ////FOR INTRO TEXT
	  	...
	  	if (curr_fuse->start_y==-1){
      		...
      		if (cur_shape == RECURSIVE) {
        		...
        		if(cur_shape==TRIFORCE){
        			BASS_ChannelPlay(sounds[26], TRUE);
        		}
        		else if(cur_shape==ONE_UP){
        			BASS_ChannelPlay(sounds[25], TRUE);
        		}
        		else if(cur_shape==MUSHROOM){
        			BASS_ChannelPlay(sounds[27], TRUE);
        		}
        		else{
              if (cur_shape == BOOM) {
                num = rand() % 2;
              }
              if (cur_shape == REGULAR) {
                num = (rand() % 3) + 11;
              }
              if (cur_shape == SPARKLER) {
                num = (rand() % 4) + 21;
              }
    ...
    for (i = 0; i < (&explosions[j])->quantity; i++) {
      ...
      if (cur_explosion->counter < cur_explosion->radius) {
        if (cur -> is_sparkler == 1) {
          cur -> translation[0] = (float)((rand() % (10)) - 4.5)/180.;
          cur -> translation[1] = (float)((rand() % (10)) - 4.5)/180.;
          cur -> translation[2] = (float)((rand() % (10)) - 4.5)/180.;
        }
        if (cur -> is_willow != 0) {
...
void setup_BASS()
{
  ...
  if (!BASS_Init(-1,44100,0,0,NULL))
    ...
    // dense boom
    sounds[0] = BASS_StreamCreateFile(FALSE, "./sound/boom/boom0.mp3", 0, 0, 0);
    sounds[1] = BASS_StreamCreateFile(FALSE, "./sound/boom/boom1.mp3", 0, 0, 0);

    // for the launch out of the nozzle
    sounds[2] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[3] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[4] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[5] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[6] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[7] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);
    sounds[8] = BASS_StreamCreateFile(FALSE, "./sound/launch/launch8.mp3", 0, 0, 0);

    // music for 'z'
    sounds[9] = BASS_StreamCreateFile(FALSE, "./sound/music/trans.mp3", 0, 0, 0);
    sounds[10] = BASS_StreamCreateFile(FALSE, "./sound/music/1812.wav", 0, 0, 0);

    BASS_ChannelSetAttribute(sounds[9], BASS_ATTRIB_VOL, .7);

    // for the regular firework
    sounds[11] = BASS_StreamCreateFile(FALSE, "./sound/regular/bang0.mp3", 0, 0, 0);
    sounds[12] = BASS_StreamCreateFile(FALSE, "./sound/regular/bang4.mp3", 0, 0, 0);
    sounds[13] = BASS_StreamCreateFile(FALSE, "./sound/regular/bang5.mp3", 0, 0, 0);

    // for nozzle sparklers
    sounds[14] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[15] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[16] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[17] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[18] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[19] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);
    sounds[20] = BASS_StreamCreateFile(FALSE, "./sound/sizzle/sizzle1.mp3", 0, 0, 0);

    // for the sparklers
    sounds[21] = BASS_StreamCreateFile(FALSE, "./sound/sparkle/bang2.mp3", 0, 0, 0);
    sounds[22] = BASS_StreamCreateFile(FALSE, "./sound/sparkle/bang3.mp3", 0, 0, 0);
    sounds[23] = BASS_StreamCreateFile(FALSE, "./sound/sparkle/new2.mp3", 0, 0, 0);
    sounds[24] = BASS_StreamCreateFile(FALSE, "./sound/sparkle/new3.mp3", 0, 0, 0);

    // for the special stuff
    sounds[25] = BASS_StreamCreateFile(FALSE, "./sound/special/1-up.wav", 0, 0, 0);
    sounds[26] = BASS_StreamCreateFile(FALSE, "./sound/special/OOT.wav", 0, 0, 0);
    sounds[27] = BASS_StreamCreateFile(FALSE, "./sound/special/smb_powerup.wav", 0, 0, 0);
