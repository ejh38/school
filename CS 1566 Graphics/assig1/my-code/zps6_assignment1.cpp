/**************************************************************************
File: zps6_assignment1.cpp
Does: Place and throw around some rectangles and circles! Have fun!
...
#include "zps6_assignment1.h"
#include "Shape.cpp"    // this is the class I created
#include <math.h>       // use this for sin and cos
...
double gravity = .0000035;        // this is just a small number I found from messing around
...
double temp_rect_width = 0.1, temp_rect_height = 0.1;          // for the temporary/cursor shape, the dim of rect
double temp_circ_radius_x = .1, temp_circ_radius_y = .13333;   // same, but for the circle

double thickness_step = .025;  // amount you adjust by when IJKL'ing

double red = 1;
double blue = 1;
double strobe = -1, strobe_amount = .005;   // for strobe

int old_mouse_x, old_mouse_y;  // for throwing
double mouse_dx, mouse_dy, dy_divisor = 12000, dx_divisor = 15000;  // to make the values reasonable for throwing
...
void glut_setup(void) {
  ...
  glutInitWindowPosition(300,100);        // make the window appear not in the very top left corner of the screen
  ...
  glutPassiveMotionFunc(my_motion);   // for throwing
  glutMotionFunc(my_motion);          // for throwing
...
void my_display(void) {
  ...
  for (int i = 0; i < (25 + temp_display); i++) {
    if (shapes[i].get_type() == 1) {          // type 1 is rectangle
    ...
    else if (shapes[i].get_type() == -1) {  // type -1 is circle
...
void my_reshape(int w, int h) {
  ...
  temp_circ_radius_y = window_w * temp_circ_radius_x / window_h;    // this keeps the proportion a circle, 
                                                                    // but it won't affect the already placed ones
...
void my_motion(int x, int y)
{
  ...
  // I use this for throwing the shapes
...
void my_mouse(int b, int s, int x, int y) {
  ...
  case GLUT_LEFT_BUTTON:
    if (s == GLUT_UP)  {      // let go of the button
      ...
      if (mouse_dx > .9 || mouse_dx < -.9)                    // if you've moved the mouse left or right
        shapes[count].set_x_velocity(-mouse_dx/dx_divisor);   // start with some x_velocity
      else
        shapes[count].set_x_velocity(0);                      // otherwise start with none
