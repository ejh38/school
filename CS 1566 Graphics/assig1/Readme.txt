...
Extra credit (describe what you did, if any):
	1. 'Throwing' objects up, down, left, or right before releasing the mouse button will result in the shapes having initial velocity (x, y, or both).
		- There is a greater effect to the left and right than up or down, but with a quick mouse drag you can send objects flying down or up.
	...
	4. Pressing the 'i,j,k,l' buttons adjust the width and height of the current shape (rectangle or circle).
		...
		- The width/height are stored seperately for circles and radius, and will effect all future shapes of that type.
	5. Pressing the 'e' key will 'exolode' the shapes, setting them off with random x and y velocity.
...
I made a few design choices that I'll highlight here so you don't consider them bugs:
	...
	- Reducing the width or height has no effect on the bounciness, although there is an apparent visual effect (since the objects are then smaller and appear to go higher, or bigger and seem to bounce less).
	- 'Throwing' objects to the left and right has more of an effect than up or down.
	- Objects do not deaccelerate in the x direction until they hit a wall, or are motionless in the y direction (because of this, objects tend to clump up towards the left or right wall after many explosions).
	- At large radius, the un-roundedness of circles is more apparent. I chose a smaller amount of lines to make my circle (60 v 360) and this is a consequence.
	- Lastly, clear the screen, turn on strobe, make some shapes of different size, then explode a few times to watch a pretty neat screensaver-type image.