...
short rect_or_circle = 1;		// 1 for rect, -1 for circle

Shape* shapes = new Shape[25];
int curr = 0;	// counter to go between 1-25

int i = 0;		// for the for loop later on
...
void my_display(void) {
	...
	if (shap.get_type() == 1)
  {
	...
	glRectf( shap.get_x()/window_w, shap.get_y()/window_h, shap.get_x()/window_w + my_rect_w, shap.get_y()/window_h + my_rect_h);
  }
  else {
	  glColor3f(1,0,0);
	  glBegin(GL_TRIANGLE_FAN);
	  glVertex2d( shap.get_x()/window_w, shap.get_y()/window_h);	// centered on the mouse click
	  for (i = 0; i < 360; i++)
	  {
		  glVertex2d(shap.get_x()/window_w + cos(i)*circle_radius_x, shap.get_y()/window_h + sin(i)*circle_radius_y);   
	  }
	  glEnd();
  }
...
void my_reshape(int w, int h) {
  ...
  window_w = w;
  window_h = h;
	circle_radius_y = window_w*circle_radius_x/window_h;		// this keeps the proportion a circle
  return;
}
