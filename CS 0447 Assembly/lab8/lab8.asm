...
# zps6@pitt.edu
# Lab 8
# A small game to eat the green LEDs!
# Saved registers used:
# $s1 has x position of Red LED,
# $s2 has y position of Red LED,
# $s3 has score (which varies 0-5).
# All other registers are replaced
# after use in functions.
############################

.data
win_message:	.asciiz "You win! You got all the green dots!"
...
.text
  ...
  syscall							# set the seed with that time				
	...
	li			$t0, 5				# set the counter to 5
	la			$t1, green_x		# base address we will be storing to
init_green_x_loop:
	beq			$t0, 0, init_green_x_exit
	
	li			$a0, 1				# set the seed id	
	li			$a1, 128			# This will give range [0-127]		
	jal			get_rand_int		# the number is in $v0
	move		$a2, $v0			# store the value of the int
	
	li			$a0, 0				# check the x position
	move		$a1, $v0			# this is the value to check
	jal			check_for_match		# see if there is already an LED there
	bne			$v0, -1, init_green_x_loop		# if there is, then pick another spot
...
init_green_y_loop:
	...
	li			$a0, 1				# set the seed ID		
	li			$a1, 8				# This will give range [0-7]		
	jal			get_rand_int		# the number is in $v0
	move		$a2, $v0			# store the value of the int
	
	li			$a0, 1				# check the y position
	move		$a1, $v0			# this is the value to check
	jal			check_for_match		# see if there is already an LED there
	bne			$v0, -1, init_green_y_loop		# if there is, then pick another spot
...
center_pressed:
	...
	beq			$s2, 5, _exit		# if the score is 5, then exit!
