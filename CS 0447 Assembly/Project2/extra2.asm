# Zach Sadler
# Extra credit 2
# Attempt at random frogs,
# I don't like how much it slows
# animation down. Not finishing it.
...
down_pressed:
	...
	j			move_done
center_pressed:	
	j			game_over				# put the game board stuff here
...
hit_wall_try_again:
	...
	add			$a1, $v1, $s2
	jal			getLED
	beq			$v0, 2, hit_wall_fail
	beq			$v0, 1, hit_wall_fail
...
rand_frog:
	...
	li			$v0, 42	
  syscall
	move		$v1, $v0			# not it's stored in $v1
  syscall
	move		$a0, $v0
	move		$a1, $v1
	jal			getLED
	bne			$v0, 3, no_find		# if it's not green, we didn't find one.
	
	move		$t0, $a0
	move		$t1, $a1			# temporarily store these
	move		$t3, $t0
	move		$t4, $t1
...
try_frog_again:	
	...
	li			$a1, 3				# This will give range [0.1]		
	jal			get_rand_int		# the number is in $v0
	subi		$v0, $v0, 1			# sub one to give range [-1, 1]	
	add			$t0, $v0, $t0		# same for y
	
	li			$a1, 3					
	jal			get_rand_int		
	subi		$v0, $v0, 1
	add			$t1, $v0, $t1		# add the offset
	
	move		$a0, $t0			# get ready
	move		$a1, $t1
	blt			$a0, 0, try_frog_again
	blt			$a1, 0, try_frog_again
	bge			$a0, 64, try_frog_again
	bge			$a1, 64, try_frog_again
	jal			getLED
	bne			$v0, 0, try_frog_again
	li			$a2, 3				# and then print green
    jal			setLED	
