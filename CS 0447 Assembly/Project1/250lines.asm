...
first_time: lw			$s2, word_tbl		# load 'computer' as word
	move		$a0, $s2
	li			$s0, 0				# set the tally score to 0
	j			word_picked			# skip picking a random word
outer_loop:	jal			pick_rand_word		# returns the address of the word to $a0,
 	move		$s2, $a0			# save the word's address to $s2
word_picked: li			$s4, 3				# reset the number of free letters
...
game_loop: jal			check_win
  ...
  syscall							# read a character to $v0
  	...
  	beq			$s1, $0, round_lost
	j game_loop  
forfeit_round: la			$a0, forfeit		# tell them they gave up
	jal			print
	li			$s1, 0				# set round score to zero
	j			round_over
free_letter: beq			$s4, $0, have_none	# if they have none, jump
  ...
  syscall	
	...
	jal			add_free_letter
	j			game_loop
have_none:		la			$a0, no_free		# tell them they're out
	jal			print
	j			game_loop
round_won: la			$a0, won_round
	jal			print
 	j			round_over
round_lost:  	la			$a0, lost_round
 	jal			print
 	j			round_over
round_over: la			$a0, correct_word	# 'correct word was'
 	jal			print
...
guess_loop: 	beq			$t0, $0, guess_right
...
pick_rand_word:		addi		$sp, $sp, -12
  ...
  syscall							# generates a random int 0-9	
	...
	addi		$sp, $sp, 12
	jr			$ra
get_length:	addi		$sp, $sp, -12		# make room for the three variables used
	sw			$t0, 0($sp)
	sw			$t1, 4($sp)
	sw			$ra, 8($sp)			# store the variables we'll be using
	li			$t0, 0				# $t0 is the running counter to find the length
get_length_loop: lb			$t1, ($a0)			# loads one charactar from the current string index
...
replace_word:	addi		$sp, $sp, -12
	sw			$t0, 0($sp)
	sw			$t1, 4($sp)
	sw			$ra, 8($sp)
	la			$t0, word_replaced
	li			$t1, 0x5F20			# load the hex value for " _"
replace_loop: sh			$t1, ($t0)			# store " _"
...
extra_letters:	lh			$t1, ($t0)
	beq			$t1, $0, no_more_extra	
	sh			$0, ($t0)
	addi		$t0, $t0, 2
	j			extra_letters		
no_more_extra:		la			$a2, word_replaced	# for the return value
	move		$a0, $a2
...
star_loop: beq			$t1, $0, star_exit	# if counter is zero, get out
...
add_free_letter_loop: beq			$t0, $t3, add_free_letter_exit	# if working length = 0, exit		
...
found_spot:	add			$t0, $t3, $s2		# move down the actual word
	lb			$t1, 0($t0)
	sb			$t1, 0($a0)
 add_free_letter_exit: lw			$t0, 0($sp)
 	lw			$t1, 4($sp)
 	lw			$t2, 8($sp)
 	lw			$t3, 12($sp)
 	lw			$ra, 16($sp)
...
check_loop: beq			$t1, $t2, check_exit	# if currPosition = length, exit ** only way to get out
	...
	j			check_loop
check_success: move		$a2, $t2				# store value of $t2
...
check_loop_star: move		$t2, $a2				# move back
	move		$a0, $a3				# move back
	addi		$a0, $a0, 1				# move a byte down the address
	addi		$t2, $t2, 1				# increment currPosition
	j			check_loop				# jump back in to keep looking
check_exit:beq			$t4, 1, good_guess		# if they're right, tell them
	beq			$t4, 0, bad_guess		# if they're wrong, tell them too
good_guess:la			$a0, yes
	jal			print
	j			check_return
bad_guess:la			$a0, no
	jal			print
	addi		$s1, $s1, -1	
check_return: lw			$t0, 0($sp)
	lw			$t1, 4($sp)
	lw			$t2, 8($sp)
	lw			$t3, 12($sp)
	lw			$t4, 16($sp)
...
check_win_loop: beq			$t0, $t3, check_win_exit	# if reach the end, get out		
 	lb			$t1, 0($a0)					# load character
 	beq			$t1, $t2, no_win	 		# if character = "_", haven't won yet
 	addi		$t3, $t3, 1			# increment length counter
 	addi		$a0, $a0, 2			# add 2 bytes to the address
 	j			check_win_loop
no_win: li			$v0, 0				# no win
check_win_exit: beq			$v0, 1, round_won
	lw			$t0, 0($sp)
	lw			$t1, 4($sp)
	lw			$t2, 8($sp)
	lw			$t3, 12($sp)
